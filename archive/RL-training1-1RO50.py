...
def execTask(taskname,limitTrial,mywin, animal_ID,session):
    ...
    while trial <= limitTrial:
        for rand_stim in choice:
            ...
            if rand_stim == 0:
                reward_grating = left_grating
                penalty_grating = right_grating
            elif rand_stim == 1:
                reward_grating = right_grating
                penalty_grating = left_grating
            
            # drawing gratings
            reward_stim.draw(mywin)
            reward_grating.draw(mywin)
            if trial % 2 == 0:
                penalty_stim.draw(mywin)
                penalty_grating.draw(mywin)
